...
bool Socket::bind(string addr)
  {
   // zmq_bind expects const char[]
   char arr[];
   StringToCharArray(addr, arr);         // write into char[]
   // ensure null-termination
   int n = ArraySize(arr);
   if(n == 0 || arr[n-1] != 0) { ArrayResize(arr, n + 1); arr[n] = 0; }
...
bool Socket::monitor(string addr,int events)
  {
   // zmq_socket_monitor expects const char[]
   char arr[];
   // Convert to char[]; ensure null-termination
   StringToCharArray(addr, arr);
   int n = ArraySize(arr);
   if(n == 0 || arr[n-1] != 0)
     {
